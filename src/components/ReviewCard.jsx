...
export default function ReviewCard({review, handleDeleteReview, handleSaveReview}) {
    ...
    return (
        ...
        {isEditing && <div >
            <input className='review-card-input' type="text" value={editedReview} onChange={(e) => setEditedReview(e.target.value)} />
        </div>}
                <p className={`review-card-p ${isEditing ? 'hidden' : ''}`}>{review.content}</p>
                ...
                {user._id === review.userId._id && <div className='review-user-actions'>
                {isEditing ? (
                    <button onClick={() => {
                ...
                <button onClick={() => handleDeleteReview(review._id, review.lineupId)}><RiDeleteBin5Line /></button>
                </div>}
                </div>
              </div>
    )
