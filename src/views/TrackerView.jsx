  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `https://api.henrikdev.xyz/valorant/v1/account/${playerName}/${tagPlayer}`
      );
      setPlayer(response.data);
      const response2 = await axios.get(
        `https://api.henrikdev.xyz/valorant/v1/mmr/eu/${playerName}/${tagPlayer}`
      );
      ...
      <div className="general-form-edit">
        ...
        {player && playerInfo.data && (
          <div className="tracker-info">
            <h3>
              {player.data.name} {player.data.tag}
            </h3>
            <div className="tracker-info-1">
              <p>
                Account level: <b>{player.data.account_level}</b>
              </p>
              <p>
                Rank: <b>{playerInfo.data.currenttierpatched}</b>
              </p>
              <img src={playerInfo.data.images.small} alt={player.data.name} />
            </div>
            <img
              className="tracker-img"
              src={player.data.card.large}
